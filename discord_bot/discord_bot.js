...
const BAD_WORDS = JSON.parse(fs.readFileSync('profanity-list.json', 'utf8'));
...
client.on('messageCreate', async message => {
    ...
    try {
        if (containsBadWords(message.content)) {
            ...
            if (warningCounts[userId] > 3) {
                try {
                    await message.member.ban({ reason: "Exceeded maximum warnings (3)" });
                    await message.channel.send(`${message.author} has been banned for exceeding 3 warnings!`);
                } catch (error) {
                    console.error(`Failed to ban user: ${error}`);
                }
            } else {
                await message.channel.send(`${message.author}, please watch your language! Warning ${warningCounts[userId]}/3`);
            }
        }
    } catch (error) {
                ...
                for (const [_, message] of messages) {
                    ...
                    if (containsBadWords(message.content)) {
                        ...
                        if (warningCounts[userId] > 3) {
                            try {
                                await message.member.ban({ reason: "Exceeded maximum warnings (3)" });
                                await channel.send(`${message.author} has been banned for exceeding 3 warnings!`);
                            } catch (error) {
